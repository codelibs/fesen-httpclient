...
import org.codelibs.fesen.opensearch.cluster.coordination.PublishClusterStateStats;
...
import org.codelibs.fesen.opensearch.core.indices.breaker.CircuitBreakerStats;
...
import org.codelibs.fesen.opensearch.index.shard.IndexingStats.Stats.DocStatusStats;
...
import org.codelibs.fesen.opensearch.index.store.remote.filecache.FileCacheStats;
...
import org.codelibs.fesen.opensearch.monitor.jvm.JvmStats.MemoryPoolGcStats;
...
import org.codelibs.fesen.opensearch.tasks.SearchShardTaskCancellationStats;
import org.codelibs.fesen.opensearch.tasks.SearchTaskCancellationStats;
...
import org.codelibs.fesen.opensearch.ratelimitting.admissioncontrol.stats.AdmissionControllerStats;
import org.codelibs.fesen.opensearch.node.ResponseCollectorService.ComputedNodeStats;
...
public class HttpNodesStatsAction extends HttpAction {
    ...
    protected NodeStats parseNodeStats(final XContentParser parser, final String nodeId) throws IOException {
        ...
        RepositoriesStats repositoriesStats = null;
        ...
        while ((token = parser.currentToken()) != XContentParser.Token.END_OBJECT) {
            if (token == XContentParser.Token.FIELD_NAME) {
                ...
                if ("mode".equals(fieldName)) {
                    mode = SearchBackpressureMode.fromName(parser.text());
                }
            }
            parser.nextToken();
        }
    ...
    protected SegmentReplicationRejectionStats parseSegmentReplicationRejectionStats(final XContentParser parser) throws IOException {
        ...
        while ((token = parser.currentToken()) != XContentParser.Token.END_OBJECT) {
            if (token == XContentParser.Token.FIELD_NAME) {
                ...
                if ("total_rejected_requests".equals(fieldName)) {

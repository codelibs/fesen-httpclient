...
import org.codelibs.fesen.opensearch.index.compositeindex.datacube.startree.utils.StarTreeUtils;
...
import org.codelibs.fesen.opensearch.search.aggregations.NonCollectingAggregator;
...
import org.codelibs.fesen.opensearch.search.aggregations.bucket.filterrewrite.RangeAggregatorBridge;
...
import static org.codelibs.fesen.opensearch.search.aggregations.bucket.filterrewrite.AggregatorBridge.segmentMatchAll;
...
public class RangeAggregator extends BucketsAggregator implements StarTreePreComputeCollector {
    ...
    public InternalAggregation buildEmptyAggregation() {
        ...
        List<org.codelibs.fesen.opensearch.search.aggregations.bucket.range.Range.Bucket> buckets = new ArrayList<>(ranges.length);
        for (int i = 0; i < ranges.length; i++) {
            Range range = ranges[i];
            org.codelibs.fesen.opensearch.search.aggregations.bucket.range.Range.Bucket bucket = rangeFactory.createBucket(
                range.key,
                range.from,
                range.to,
                0,

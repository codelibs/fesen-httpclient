...
/**
 * Namespace for the rare-terms bucket order. The aggregator itself is node-side and is not carried over.
 *
 * @opensearch.internal
 */
public final class LongRareTermsAggregator {

    /** Rare terms are ordered by ascending count, then ascending key. */
    static final BucketOrder ORDER = BucketOrder.compound(BucketOrder.count(true), BucketOrder.key(true));

    private LongRareTermsAggregator() {
    }
}

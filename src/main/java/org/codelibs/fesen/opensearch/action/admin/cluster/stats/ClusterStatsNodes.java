...
public class ClusterStatsNodes implements ToXContentFragment {
    ...
    public static class OsStats implements ToXContentFragment {
        ...
        final org.codelibs.fesen.opensearch.monitor.os.OsStats.Mem mem;
    ...
    public static class JvmStats implements ToXContentFragment {
        ...
        private JvmStats(List<NodeInfo> nodeInfos, List<NodeStats> nodeStatsList) {
            ...
            for (NodeStats nodeStats : nodeStatsList) {
                org.codelibs.fesen.opensearch.monitor.jvm.JvmStats js = nodeStats.getJvm();
                if (js == null) {
                    continue;
                }
        ...
        IngestStats(final List<NodeStats> nodeStats) {
            ...
            for (NodeStats nodeStat : nodeStats) {
                if (nodeStat.getIngestStats() != null) {
                    for (Map.Entry<String, List<org.codelibs.fesen.opensearch.ingest.IngestStats.ProcessorStat>> processorStats : nodeStat.getIngestStats()
                        .getProcessorStats()
                        .entrySet()) {
                        pipelineIds.add(processorStats.getKey());
                        for (org.codelibs.fesen.opensearch.ingest.IngestStats.ProcessorStat stat : processorStats.getValue()) {

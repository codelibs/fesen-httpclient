 ...
 * Use {@link org.codelibs.fesen.opensearch.index.engine.exec.coord.CatalogSnapshotManager#acquireSnapshot()} to obtain
 * a reference-counted handle, and close the returned {@link org.codelibs.fesen.opensearch.common.concurrent.GatedCloseable}
...
public abstract class CatalogSnapshot implements Writeable, Cloneable {
    ...
    /**
     * Builds a list of {@link org.codelibs.fesen.opensearch.index.engine.Segment} from this catalog snapshot's segments.
     * <p>
     ...
     */
    public List<org.codelibs.fesen.opensearch.index.engine.Segment> buildEngineSegments(Map<String, String> lastCommitData, Sort indexSort) {
        boolean isCommitted = resolveIsCommitted(lastCommitData);
        List<org.codelibs.fesen.opensearch.index.engine.Segment> result = new java.util.ArrayList<>(getSegments().size());
        for (Segment dfSeg : getSegments()) {
            org.codelibs.fesen.opensearch.index.engine.Segment seg = new org.codelibs.fesen.opensearch.index.engine.Segment("_" + Long.toHexString(dfSeg.generation()));
            seg.search = true;
        ...
        result.sort(java.util.Comparator.comparingLong(org.codelibs.fesen.opensearch.index.engine.Segment::getGeneration));
        return result;
    }
        ...
        for (Segment segment : getSegments()) {
            for (Map.Entry<String, WriterFileSet> entry : segment.dfGroupedSearchableFiles().entrySet()) {
                ...
                for (String file : entry.getValue().files()) {
                    String dirFile = org.codelibs.fesen.opensearch.index.store.FileMetadata.serialize(formatName, file);
                    try {

...
public final class JacksonExceptionTranslator {
    ...
    public static IOException translateToIOExceptionOrRethrowReturning(JacksonException ex) {
        if (ex instanceof UnexpectedEndOfInputException) {
            return new org.codelibs.fesen.opensearch.tools.jackson.core.UnexpectedEndOfInputException(ex.getMessage(), ex);
        } else if (ex instanceof InputCoercionException) {
            return new org.codelibs.fesen.opensearch.tools.jackson.core.InputCoercionException(ex.getMessage(), ex);
            ...
            return new org.codelibs.fesen.opensearch.tools.jackson.core.StreamConstraintsException(ex.getMessage(), ex);
        } else {
